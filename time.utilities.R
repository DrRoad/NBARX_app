...
next_bizdays <-function(posixct_date,n_days=1){
  tz <- attr(posixct_date,"tzone")
  timedate_date <- as.timeDate(posixct_date)
  day_count <- 0
  while(day_count<n_days){
    timedate_date <- timedate_date + 86400
    day_count <- day_count + isBizday(timedate_date)
  }
  posixct_date <- as.POSIXct(timedate_date,tz=tz)
  return(posixct_date)
}

time_of_day_sec <- function(posixct_date){
  return(hour(posixct_date)*3600 + minute(posixct_date)*60 + second(posixct_date))
}


seq_only_bizhours <- function(posixct_start_date, posixct_end_date, by, exchange_hours){
  posixct_seq_all_days <- seq(posixct_start_date,posixct_end_date,by)
  tz <- attr(posixct_seq_all_days,"tzone")
  timedate_seq_all_days <- as.timeDate(posixct_seq_all_days)
  open_time <- time_of_day_sec(exchange_hours$open_time)
  close_time <- time_of_day_sec(exchange_hours$close_time)
  time_of_day <- time_of_day_sec(timedate_seq_all_days)
  timedate_seq_only_bizhours <- timedate_seq_all_days[isBizday(timedate_seq_all_days) & time_of_day>=open_time & time_of_day<=close_time]
  return(as.POSIXct(timedate_seq_only_bizhours,tz=tz))
}
...
plot.intra <- function(x,ex_hours,date_end=NULL,tick_dist="30 min",new=TRUE,x_shift=0,...){
  ...
  if(new){
    par(mar=c(4,4,4,1))
    plot(x_shift+t_plot[1:length(x)],x,xaxt="n",...)
